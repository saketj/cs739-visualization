	...
	"nodes": [
		{
			...
			"paper-highlights": "<ul>" + 
	  				"<li><b>Data:</b> The authors looked at 100000 drives including HPC clusters at Los Alamos and the Pittsburgh Supercomputer Center, as well as several unnamed internet services providers. The drives had different workloads, different definitions of failure and different levels of data collection.</li>" + 
	  				"<li><b>Vendor MTBF reliability: </b> While the MTTFs on the data sheets suggested an annual failure rate of no more than 0.88%, in the field, annual disk replacement rates typically exceed 1%, with 2-4% common and up to 13% observed on some systems. </li>" +
	  				"<li><b>No bathtub curve:</b> The disk failures were found to increase constantly with age, rather than setting in after a nominal life time of five years.</li> " + 
	  				"<li><b>High-end enterprise drives versus consumer drives:</b> The authors observed little difference in replacement rates between SCSI, FC and SATA drives, potentially an indication that disk-independent factors, such as operating conditions, affect replacement rates more than component specific factors </li>" + 
	  				"<li><b>Independence of drive failures in an array?</b> The distribution of time between disk replacements exhibits decreasing hazard rates, that is, the expected remaining time until the next disk was replaced grows with the time it has been since the last disk replacement.  one array drive failure means a much higher likelihood of another drive failure. The longer since the last failure, the longer to the next failure.</li> " +
				"</ul>",

			"paper-observations": "<ul>" +
						"<li>The devices we characterized in this study showed variation both within a block and over time in terms of power consumption, latency, and error rates.</li> " + 
						"<li>Our data also show that the values manufacturers provide in publicly available datasheets often tell only part of the story, and that actual performance can be significantly worse and highly variable.</li> " + 
						"<li>Our application case studies demonstrate that by looking beyond the datasheets manufacturers provide, we can make significant improvements to flash-based storage devices.</li> " + 
						"<li>Exploiting two of the effects we measured enabled us to significantly decrease latency for critical IO requests and extend the effective lifetimes of chips.</li> " + 
					  "</ul>"
		...
		},
		{
			...
			"paper-highlights": "<ul>" + 
					  "<li><b>Data:</b> The authors analyzed the error logs on over 50,000 arrays covering 1.53 million enterprise and consumer drives disks.</li>" + 
					  "<li>Latent Sector Errors (LSE) is defined as: when a particular disk sector cannot be read or written, or when there is an uncorrectable ECC error. Any data previously stored in the sector is lost.</li>" + 
					  "<li><b>File system implications:</b> File systems rely on disk-based data structures to keep track of your stuff. One of the key findings of the team is that disk errors tend to congregate near each other. File systems that replicate critical data across the disk are much less likely to lose data than those, like ReiserFS, place critical structures in one contiguous area.</li>" + 
					  "<li><b>Enterprise vs consumer drive:</b> The higher LSE rate increase for aging consumer drives suggests that enterprise drives are higher quality. Or perhaps their error correction is better.</li>" + 
				 	  "<li><b>Several factors that contribute to LSE: </b>" + 
				 	  		"<ul>" + 
				 	  			"<li>Size: As disk size increases, so does the fraction of disks with LSE.</li>" +
				 	  			"<li>Age: LSE rates climbed with age. 20% of some – but not all – consumer disks had LSE after 24 months. Rates climbed faster for consumer drives than for enterprise drives.</li>" +
				 	  			"<li>Vendor: They also found that some vendors had much higher LSE than others.</li>" +
				 	  			"<li>Errors: A drive that develops one error is much more likely to develop a second. The second error is likely to be close to the first error. Once a drive develops an error, both enterprise and consumer drives are equally likely to develop a 2nd error.</li>" +
				 	  		"</ul><li>" +  
				"</ul>",
		...
		},
		{
			...
			"paper-highlights": "<ul>" + 
				  "<li><b>Data:</b> The authors looked at that company’s experience with more than 100,000 serial and parallel ATA consumer-grade HDDs running at speeds of 5400 to 7200 rpm. The disks were at least nine different models from seven different manufacturers, with sizes from 80 to 400 GB. </li>" + 
				  "<li><b>Age:</b> Disks have an annualized failure rate (AFR) of 3% for the first three months, dropping to 2% for the first year. In the second year the AFR climbed to 8% and stayed in the 6% to 9% range for years 3-5</li>" + 
				  "<li><b>Utilization: </b>It is commonly expected that high utilization correlates with high failure rates. However, the authors observed that only very young and very old disc groups appear to show the expected behavior. It is possible that failure modes that associated with higher utilization are more prominent early in drive’s lifetime. the drives that survive the infant mortality phase are the least susceptible to that failure mode. High correlation between utilization and failures has been based on extrapolations from manufacturers’ accelerated life experiments. Those experiments are likely to better model early life failure characteristics and as such they agree with the trend we observe for the young age groups</li>" + 
				  "<li><b>Temperature: </b>Temperature is often quoted as the most important environmental factor affecting disk drive reliability. Previous studies have indicated that temperature deltas as low as 15C can nearly double disk drive failure rates. Temperature effects only for the high end of our range and especially for older drives. In the lower and middle temperature ranges, higher temperatures are not associated with higher failure rates. The authors conclude that at moderate temperature ranges it is likely that there are other effects which affect failure rates much more strongly than temperatures do.</li>" + 
				  "<li><b>SMART: </b> 36% of the failed drives did not exhibit a single SMART-monitored failure. They concluded that SMART data is almost useless for predicting the failure of a single drive.</li>" + 
				"</ul>",
		...
		},
		{
			...
			"paper-highlights": "<ul>" + 
				  "<li><b>Data: </b>The authors studied corruption instances that were logged in tens of thousands of storage systems for a period of 41 months starting in January 2004. These systems belonged to a range of different models, ran different versions of storage-controller software and contain many different models or versions of hardware components. The sample consisted of 1.53 million disk drives of 14 disk families and 31 distinct models.</li>" + 
				  "<li>Albeit not as common as latent sector errors, data corruption does happen; we observed more than 400,000 cases of checksum mismatches. For some drive models as many as 4% of drives develop checksum mismatches during the $17$ months examined. Similarly, even though they are rare, identity discrepancies and parity inconsistencies do occur. Protection offered by checksums and block identity information is therefore well-worth the extra space needed to store them.</li>" + 
				  "<li>A significant number (8% on average) of corruptions are detected during RAID reconstruction, creating the possibility of data loss. In this case, protection against double disk failures is necessary to prevent data loss. More aggressive scrubbing can speed the detection of errors, reducing the likelihood of an error during a reconstruction.</li>" + 
				  "<li>Although, the probability of developing a corruption is lower for enterprise class drives, once they develop a corruption, many more are likely to follow. Therefore, replacing an enterprise class drive on the first detection of a corruption might make sense (drive replacement cost may not be a huge factor since the probability of first corruption is low).</li>" + 
				  "<li>Some block numbers are much more likely to be affected by corruption than others, potentially due to hardware or firmware bugs that affect specific sets of block numbers. RAID system designers might be well advised to use staggered stripes such that the blocks that form the stripe are not stored at the same or nearby block number.</li>" + 
				  "<li>Strong spatial locality suggests that redundant data structures should be stored distant from each other.</li>" + 
				  "<li>The high degree of spatial and temporal locality also begs the question of whether many corruptions occur at the exact same time, perhaps when all blocks are written as part of the same disk request. This hypothesis suggests that important or redundant data structures that are used for recovering data on corruption should be written as part of different write requests spaced over time.</li>" + 
				  "<li>Strong spatial and temporal locality (over long time periods) also suggests that it might be worth investigating how the locality can be leveraged for smarter scrubbing, e.g. trigger a scrub before it's next scheduled time, when probability of corruption is high or selective scrubbing of an area of the drive that's likely to be affected.</li>" + 
				  "<li>Failure prediction algorithms in systems should take into account the correlation of corruption with other errors such as latent sector errors, increasing the probability of one error when an instance of the other is found. </li>" + 
				"</ul>",	
		...
		},
		{
			...
			"paper-highlights": "<ul>" + 
				  "<li><b>Data:</b> The authors  analyzed the storage logs collected from about 39,000 storage systems commercially deployed at various customer sites. The data set covers a period of 44 months and includes about 1,800,000 disks hosted in about 155,000 storage shelf enclosures. Furthermore, our data covers a wide range of storage system classes, including near-line (backup), low-end, mid-range, and high-end systems.</li>" + 
				  "<li>Disk failures contribute to “only” 20-55 percent of storage subsystem failures.</li>" + 
				  "<li>Physical interconnect failures are a significant contributor – anywhere from 27-68% – of storage subsystem failures. Thus, to build highly reliable and available storage systems, only using resiliency mechanisms targeting disk failures (e.g., RAID) is not enough. We also need to build resiliency mechanisms such as redundant physical interconnects and self-checking protocol stacks to tolerate failures in these storage components.</li>" + 
				  "<li>Subsystem failure rates that use the same disk models show similar disk failure rates – but the subsystem failure rates vary significantly.</li>" + 
				  "<li>Each individual storage subsystem failure type and storage subsystem failure as a whole exhibit strong correlations, (i.e. after one failure, the probability of additional failures of the same type is higher). In addition, failures also exhibit bursty patterns in time distribution, (i.e. multiple failures of the same type tend to happen relatively close together). These results motivate a revisiting of current resiliency mechanisms such as RAID that assume independent failures. These results also motivate development of better resiliency mechanisms that can tolerate multiple correlated failures and bursty failure behaviors.</li>" + 
				  "<li>Storage subsystems configured with two independent interconnects experienced much (30-40%) lower AFRs than those with a single interconnect. This result indicates the importance of interconnect redundancy in the design of reliable storage systems.</li>" + 
				  "<li>Interconnect and protocol failure rates are much more bursty than disk failures. Some 48% of overall subsystem failure arrive at the same shelf within 10,000 seconds (~ 3 hours) of the previous failure.</li>" + 
				  "<li>As interconnect failures are so bursty, resilience mechanisms beyond RAID are required to achieve subsystem availability.</li>" + 
				"</ul>",
		...
		},
		{
			...
			"paper-highlights": "<ul>" + 
				  "<li><b>Data: </b>Studied the same date as by Bairavasundaram et. al.</li>" + 
				  "<li>The first part of the drive shows a higher concentration of errors than the remainder of the drive. Depending on the model, between 20% and 50% of all errors are located in the first 10% of the drive’s logical sector space. Similarly, for some models the end of the drive has a higher concentration. The authors speculate the areas of the drive with an increased concentration of errors might be are areas with different usage patterns, e.g. filesystems often store metadata at the beginning of the drive.</li>" + 
				  "<li><b>Does utilization matter?</b> The authors speculate that these areas that see a higher utilization  and the number of LSEs it develops correlate with either the number of reads or the number of writes that a drive sees. However, in other research at Google by Pinheiro et. al. they didn’t find such correlation. </li>" + 
				  "<li><b>Common distance between errors:</b> Between 20–60% of all errors have a neighbor within a distance of less than 10 sectors in logical sector space. However, we also observe that almost all models have pronounced “bumps” (parts where the CDF is steeper) indicating higher probability mass in these areas.</li>" + 
			  	  "<li>Errors that are close in space are also close in time</li>" + 
				"</ul>",		
			"paper-observations": " "
		},
		{
